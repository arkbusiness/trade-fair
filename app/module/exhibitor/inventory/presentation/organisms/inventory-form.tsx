...
const validationSchema = (isEdit: boolean) =>
  yup.object().shape({
    ...
    images: !isEdit
      ? yup.array<File>().min(1, 'Images are required')
      : yup.array<File>(),
    imageUrls: yup.array(),
    quantity: yup
      .number()
...
export const InventoryForm = ({ isReadOnly, inventory }: InventoryForm) => {
  ...
  const form = useForm<InventoryFormValues>({
    ...
    mode: 'onBlur',
    resolver: yupResolver(validationSchema(!!inventory))
  });
  ...
  const buildFormData = (data: InventoryFormValues): FormData => {
    ...
    formData.append('availableFrom', formatISO(data.availableFrom!));
    formData.append('availableTo', formatISO(data.availableTo!));
    ...
    (data.imageUrls ?? []).forEach((imgUrl) => {
      formData.append('imageUrls', imgUrl);
    });
    ...
    if (validAttrs.length > 0) {
      validAttrs.forEach((attr, attrIndex) => {
        formData.append(`customAttrs[${attrIndex}][key]`, attr.key);
        formData.append(`customAttrs[${attrIndex}][value]`, attr.value);
      });
    }

    // Add product category ID
    if (data.productCategoryId?.id) {
      formData.append('productCategoryId', data.productCategoryId.id);
    }
  ...
  const uploadedImages = watchedImageUrls ?? [];
                      ...
                      {uploadedImages.map((image) => (
                        ...
                        >
                          ...
                          {!isReadOnly && (
                            <button
                              type="button"
                              className="absolute -top-4  bg-tertiary rounded-full text-background w-6 h-6 flex items-center justify-center cursor-pointer self-center z-10"
